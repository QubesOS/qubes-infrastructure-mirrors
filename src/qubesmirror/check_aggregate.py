...
parser.add_argument(
    "paths", metavar="RESULT", type=pathlib.Path, nargs="+", help="path to result.json"
)
...
class Check:
    ...
    def __init__(self, obj):
        self.path = obj["path"]
        self.utcnow = datetime.datetime.strptime(obj["utcnow"], "%Y-%m-%dT%H:%M:%SZ")
...
class ReportFormatter:
    ...
    def _url_sort_key(url):
        ...
        if url.netloc.endswith(".qubes-os.org"):
            ...
            return ()
        return tuple(reversed(url.netloc.strip(".").split("."))), url.scheme, url.path

    def get_lines(self):
        """Generate lines for printing"""
        yield "\n".join(
            "{:3d} {}".format(i, mirror) for i, mirror in enumerate(self.mirrors)
        )

        yield "{:20s}  {}".format(
            "TIMESTAMP", " ".join("{!s:7s}".format(i) for i in range(len(self.mirrors)))
        )
    ...
    def format_cell(mirror):
        ...
        if mirror.get("status") is not None and not (200 <= mirror["status"] < 300):
...
def main(args=None):
    ...
    report = ReportFormatter(Check(json.load(path.open())) for path in args.paths)
