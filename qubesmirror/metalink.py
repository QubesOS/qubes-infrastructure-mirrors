...
class RepoMD:
    ...
    class Alternate(
        collections.namedtuple("_Alternate", ("timestamp", "size", "hashes"))
    ):
        ...
        def from_xml3(cls, xml):
            ...
            return cls(
                timestamp=int(
                    xml.xpath("./mm0:timestamp", namespaces=XML_NAMESPACES)[0].text
                ),
                size=int(
                    xml.xpath("./metalink3:size", namespaces=XML_NAMESPACES)[0].text
                ),
                hashes=tuple(
                    (e.get("type"), e.text)
                    for e in xml.xpath(
        ...
        def parse_metalink3(cls, path):
            ...
            for alternate in xml.xpath(
                "./mm0:alternates/mm0:alternate", namespaces=XML_NAMESPACES
            ):
    ...
    def _get_timestamp(self):
        ...
        return max(
            int(e.text)
            for e in xml.xpath(
                "/repomd:repomd/repomd:data/repomd:timestamp", namespaces=XML_NAMESPACES
            )
        )
...
parser.set_defaults(
    template=METALINK3, base=".", alt_max_count=0, alt_max_age=2 * 24 * 3600
)
...
def main(args=None):
    ...
    if args.old_metalink:
        repomd.load_alternates(
            args.old_metalink, max_count=args.alt_max_count, max_age=args.alt_max_age
        )
