...
class RepoMD:
    ...
    class Alternate(
        ...
        def from_xml3(cls, xml):
            ...
            return cls(
                timestamp=int(
                    xml.xpath("./mm0:timestamp", namespaces=XML_NAMESPACES)[
                        0
                    ].text
                ),
                size=int(
                    xml.xpath("./metalink3:size", namespaces=XML_NAMESPACES)[
                        0
                    ].text
                ),
                hashes=tuple(
    ...
    def _get_timestamp(self):
        ...
        return max(
            int(e.text)
            for e in xml.xpath(
                "/repomd:repomd/repomd:data/repomd:timestamp",
                namespaces=XML_NAMESPACES,
            )
        )
...
def main(args=None):
    ...
    if args.old_metalink:
        repomd.load_alternates(
            args.old_metalink,
            max_count=args.alt_max_count,
            max_age=args.alt_max_age,
        )
